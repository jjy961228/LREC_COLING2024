...
class Iterator:
    ...
    def _get_sampler(self,ds):
        return RandomSampler(ds, generator=torch.Generator().manual_seed(self.random_seed))
    ...
    def eval_en_sent1_loader(self): #stsb
        ds = Tokenization(data = self.df['sentence1'].to_numpy(),
    ...
    def eval_en_sent2_loader(self): #stsb
        ds = Tokenization(data = self.df['sentence2'].to_numpy(),
                          labels= self.df['label'].to_numpy(),
                            tokenizer=self.tokenizer,
                            max_length=self.max_length,
                            )
    ...
    def eval_cross_sent1_loader(self): #stsb
        ds = Tokenization(data = self.df['cross1'].to_numpy(),
                          labels= self.df['label'].to_numpy(),
                            tokenizer=self.tokenizer,
                            max_length=self.max_length,
                            )
    ...
    def eval_cross_sent2_loader(self): #stsb
        ds = Tokenization(data = self.df['cross2'].to_numpy(),
                          labels= self.df['label'].to_numpy(),
                            tokenizer=self.tokenizer,
                            max_length=self.max_length,
                            )
...
class CustomDataLoader:
    ...
    def reset(self):
        if self.iter_type == 'train' and (self.args.lang_type == 'en2en' or self.args.lang_type == 'en2cross'):
        ...
        elif self.iter_type == 'valid' and (self.args.lang_type == 'en2en' or self.args.lang_type == 'en2cross'):
        ...
        elif self.iter_type == 'train' and (self.args.lang_type == 'cross2cross') and self.args.method != 'ours':
        ...
        elif self.iter_type == 'train' and self.args.method == 'ours':
            self.sent1_loader = iter(self.iterator.train_en_sent1_loader())
            self.sent2_loader = iter(self.iterator.train_en_sent2_loader())
            ...
            self.cross_hard_neg_loader = iter(self.iterator.train_cross_hard_neg_loader())             
    ...
    def __next__(self):
        ...
        if self.iter_type == 'train' and self.args.method == 'ours':
            try:
                ...
                raise
            return sent1_batch, sent2_batch, en_hard_neg_batch, cross1_batch, cross2_batch, cross_hard_neg_batch
        
        else:
            try:
                sent0_batch = next(self.sent1_loader)
                sent1_batch = next(self.sent2_loader)
                hard_neg_batch = next(self.hard_neg_loader) if self.iter_type == 'train' else None
            except StopIteration:
                raise
...
class CustomCollator:
    ...
    def __next__(self):
        if self.iter_type == 'train' and self.args.method == 'ours':
            sent1_batch, sent2_batch, en_hard_neg_batch, cross1_batch, cross2_batch, cross_hard_neg_batch = next(self.data_loaders)
            input_ids = torch.stack([sent1_batch['input_ids'], sent2_batch['input_ids'], en_hard_neg_batch['input_ids'],
                                     cross1_batch['input_ids'], cross2_batch['input_ids'], cross_hard_neg_batch['input_ids']], dim=1)
            attention_mask = torch.stack([sent1_batch['attention_mask'], sent2_batch['attention_mask'], en_hard_neg_batch['attention_mask'],
                                          cross1_batch['attention_mask'], cross2_batch['attention_mask'], cross_hard_neg_batch['attention_mask']], dim=1)
            token_type_ids = torch.stack([sent1_batch['token_type_ids'], sent2_batch['token_type_ids'], en_hard_neg_batch['token_type_ids'],
                                          cross1_batch['token_type_ids'], cross2_batch['token_type_ids'], cross_hard_neg_batch['token_type_ids']], dim=1)
            return {
                    'input_ids': input_ids,
            ...
            if self.iter_type == 'train':
                ...
                input_ids = torch.stack([sent0_batch['input_ids'], sent1_batch['input_ids'], hard_neg_batch['input_ids']], dim=1)
                attention_mask = torch.stack([sent0_batch['attention_mask'], sent1_batch['attention_mask'], hard_neg_batch['attention_mask']], dim=1)
                token_type_ids = torch.stack([sent0_batch['token_type_ids'], sent1_batch['token_type_ids'], hard_neg_batch['token_type_ids']], dim=1)
                return {
                    'input_ids': input_ids,
                    'attention_mask': attention_mask,
                    'token_type_ids': token_type_ids,
                ...
                input_ids = torch.stack([sent0_batch['input_ids'], sent1_batch['input_ids']], dim=1)
                attention_mask = torch.stack([sent0_batch['attention_mask'], sent1_batch['attention_mask']], dim=1)
                token_type_ids = torch.stack([sent0_batch['token_type_ids'], sent1_batch['token_type_ids']], dim=1)
